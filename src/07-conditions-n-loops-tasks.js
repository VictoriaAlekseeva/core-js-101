...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (lettersArray.indexOf(str[i]) >= 0
      && lettersArray.indexOf(str[i]) === lettersArray.lastIndexOf(str[i])) {
      char = str[i];
      break;
    }
  }
...
function isCreditCardNumber(ccn) {
  const arr = (`${ccn}`).split('').map((x) => +x).reverse();
  const controlDigit = arr.splice(0, 1)[0];
  let sum = arr.reduce((acc, item, index) => (index % 2 === 0
    ? acc + Math.floor((item * 2) / 10) + ((item * 2) % 10)
    : acc + item), 0);
  sum += controlDigit;
  return sum % 10 === 0;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathArr.length; i += 1) {
    if (pathes.every((el) => pathArr[i] === el.split('/')[i])) path += `${pathArr[i]}/`;
  }
  return path;
}
...
function getMatrixProduct(m1, m2) {
  const product = Array(m1.length);

  for (let i = 0; i < m1.length; i += 1) {
    product[i] = Array(m2[0].length);
