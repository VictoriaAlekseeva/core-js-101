...
function getPolynom(...args) {
  return (x) => {
    const { length } = args;
    const result = args.reduce((acc, value, index) => acc + value * x ** (length - index - 1), 0);
    return result;
  };
}
...
function retry(func, attempts) {
  let restAttempts = attempts;
  if (restAttempts > 0) {
    try {
      func();
    } catch (error) {
      restAttempts -= 1;
      retry(func, restAttempts);
    }
  }
  return func;
}
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    logFunc(`${func.name}(${params.slice(1, -1)}) starts`);
    const fn = func(...args);
    logFunc(`${func.name}(${params.slice(1, -1)}) ends`);
    return fn;
  };
}
